...
- после завершения последней задачи защита снимается через три секунды: это не даёт Mac уснуть в коротком промежутке между прерыванием и заменяющей задачей;
...
- без разрешений и hooks через `libproc` находятся живые процессы `codex` и открытые ими файлы `~/.codex/sessions/**/rollout-*.jsonl`;
...
- `started_at` сверяется со временем запуска процесса-владельца, поэтому незавершённая запись от упавшего старого процесса не включает защиту после перезапуска;
- если внутренний формат Codex изменился или файл временно не читается, приложение выбирает безопасный вариант: не усыпляет Mac и показывает предупреждение, пока Codex запущен.
