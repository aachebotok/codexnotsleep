...
struct LegacyIntegrationMigratorTests {
  ...
  func removesLegacyHookWithoutReplacingSettingsSymlink() throws {
    ...
    let root = fileManager.temporaryDirectory.appending(
      path: "CodexNotSleep.LegacySymlinkTests.\(UUID().uuidString)",
  ...
  func controllerMarksSuccessfulMigrationAndDoesNotRepeatIt() throws {
    let fileManager = FileManager.default
    let root = fileManager.temporaryDirectory.appending(
      path: "CodexNotSleep.OneTimeMigrationTests.\(UUID().uuidString)",
      directoryHint: .isDirectory
    )
    defer { try? fileManager.removeItem(at: root) }
    ...
    let suiteName = "CodexNotSleep.OneTimeMigrationTests.\(UUID().uuidString)"
