...
@testable import MethamphetamineCore
...
struct LegacyHookConfigurationTests {
  ...
  func userConfigurationPathsUseInjectedHome() {
    let fixtureHome = URL(fileURLWithPath: "/tmp/methamphetamine-fixture-home", isDirectory: true)

    #expect(
      IntegrationConfigurationPaths.url(for: .codex, homeDirectory: fixtureHome).path
        == "/tmp/methamphetamine-fixture-home/.codex/hooks.json")
    #expect(
      IntegrationConfigurationPaths.url(for: .claude, homeDirectory: fixtureHome).path
        == "/tmp/methamphetamine-fixture-home/.claude/settings.json")
  }
